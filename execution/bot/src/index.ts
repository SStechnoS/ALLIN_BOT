...
bot.on('text', async (ctx) => {
  ...
  if (aiGuard.isAITrigger(text, step || '')) {
  ...
  await ctx.reply(SCRIPTS.UNKNOWN_MESSAGE)
})
...
async function handleName(ctx: BotContext, name: string) {
  ...
  await sheetsService.updateField(leadId, 'bot_activated_at', new Date().toISOString())
...
app.post('/webhook/tilda', {
  ...
  if (config.TILDA_WEBHOOK_SECRET && secret !== config.TILDA_WEBHOOK_SECRET) {
    logger.warn({ receivedSecret: secret }, 'Tilda webhook: invalid secret')
