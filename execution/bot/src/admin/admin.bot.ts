...
function isAdmin(tgId: number): boolean {
  return config.ADMIN_IDS.includes(String(tgId))
}
...
// ── Post draft for broadcast builder ─────────────────────────
type PostDraft = {
  mode: 'all' | 'noshown'
  text?: string
  media_type?: 'photo' | 'video'
  media_file_id?: string   // display only
  media_buffer?: Buffer    // actual bytes for sending
  btn_label?: string
  btn_url?: string
}

const postDrafts = new Map<number, PostDraft>()

function safePreview(text: string, maxLen = 55): string {
  const truncated = text.slice(0, maxLen) + (text.length > maxLen ? '...' : '')
  return truncated.replace(/[_*`[]/g, '\\$&')
}

function buildDraftText(draft: PostDraft): string {
...
function buildDraftKeyboard(draft: PostDraft) {
...
type PendingState =
  ...
  | { type: 'edit_video' }
  | { type: 'broadcast_field'; field: 'text' | 'media' | 'btn_label' | 'btn_url' }
  | { type: 'find_user' }
  | { type: 'test_email' }
...
// ── Build admin bot ───────────────────────────────────────────
export function createAdminBot(mainTelegram: any) {
  if (!config.ADMIN_BOT_TOKEN) {
    logger.warn('ADMIN_BOT_TOKEN not set — admin bot disabled')
    return null
  }
  ...
  async function downloadFileBuffer(fileId: string): Promise<Buffer> {
    ...
    const url = `https://api.telegram.org/file/bot${config.ADMIN_BOT_TOKEN}/${file.file_path}`
  ...
  async function sendPost(telegram: any, chatId: number | string, draft: PostDraft) {
    const inlineBtn =
      draft.btn_label && draft.btn_url
  ...
  function leadMonth(createdAt: string): string | null {
  ...
  bot.action(/^adm_stats:(.+)$/, adminOnly, async (ctx) => {
    ...
    try {
      const allLeads = await sheetsService.getAllLeads()
      ...
      logger.error({ err }, 'Admin stats period error')
  ...
  bot.action(/^adm_stats_cards:(.+)$/, adminOnly, async (ctx) => {
    ...
    try {
      ...
      for (const lead of leads) {
        ...
        const cardText =
          `${statusEmoji[lead.status] || ''} <b>${esc(lead.name)}</b>\n` +
          `📱 ${esc(lead.phone)}\n` +
          `💬 ${tgLine}\n` +
          `${lessonLine}\n` +
      ...
      logger.error({ err }, 'Admin stats cards error')
  ...
  bot.action('adm:schedule', adminOnly, async (ctx) => {
    await ctx.answerCbQuery()
    try {
      const leads = await sheetsService.findAllScheduled()
      if (!leads.length) {
  ...
  bot.action(/^adm_sched:(.+)$/, adminOnly, async (ctx) => {
    ...
    try {
      const leads = await sheetsService.findByDate(date)
      if (!leads.length) {
      ...
      logger.error({ err }, 'Admin schedule date error')
      await ctx.reply('Ошибка')
    }
  })
  ...
  bot.action(/^adm_att:(.+):(true|false)$/, adminOnly, async (ctx) => {
    ...
    try {
      await sheetsService.markAttendance(leadId, attended)
      const label = attended ? '✅ Отмечено: пришёл' : '❌ Отмечено: не пришёл'
      await ctx.editMessageReplyMarkup(undefined)
      await ctx.reply(label)
    } catch (err) {
      logger.error({ err }, 'Admin attendance error')
  ...
  bot.action(/^adm:broadcast:(all|noshown)$/, adminOnly, async (ctx) => {
    await ctx.answerCbQuery()
    const mode = ctx.match[1] as 'all' | 'noshown'
    const draft: PostDraft = { mode }
  ...
  bot.action('adm:bc_send', adminOnly, async (ctx) => {
    ...
    try {
      const leads = (await sheetsService.getAllLeads()).filter(l => {
        if (!l.tg_id) return false
        if (draft.mode === 'noshown') return l.status === 'MISSED'
        return true
      })
      ...
      for (const lead of leads) {
        try {
          ...
          logger.error({ tg_id: lead.tg_id, err: e?.message }, 'Broadcast send failed')
          fail++
        }
      }
  ...
  bot.action(/^adm_edit:(.+)$/, adminOnly, async (ctx) => {
    ...
    if (key === 'reset') {
      deleteSetting('ai_prompt')
      deleteSetting('welcome_text')
      deleteSetting('welcome_video_file_id')
      deleteSetting('email1_text')
      deleteSetting('email2_text')
    ...
    if (key === 'welcome_video') {
      const current = getSetting('welcome_video_file_id')
  ...
  bot.on('photo', adminOnly, async (ctx) => {
    ...
    if (state?.type !== 'broadcast_field' || state.field !== 'media') return
  ...
  bot.on('video', adminOnly, async (ctx) => {
    ...
    if (video.duration > 15) {
      await ctx.reply(
        `⚠️ Видео слишком длинное: ${video.duration} сек (максимум 15 сек).\nПопробуйте обрезать и отправить снова, или /cancel`
      )
      return
    }
  ...
  bot.on('text', adminOnly, async (ctx) => {
    ...
    if (state.type === 'find_user') {
      pending.delete(ctx.from!.id)
      try {
        const lead = await sheetsService.findByPhone(text)
        if (!lead) {
        ...
        logger.error({ err }, 'Admin find_user error')
    ...
    if (state.type === 'test_email') {
      ...
      try {
        await emailService.sendEmail1(text, 'Тест')
  ...
  bot.on('video_note', adminOnly, async (ctx) => {
    ...
    setSetting('welcome_video_file_id', fileId)
  ...
  bot.catch((err, ctx) => {
    logger.error({ err, userId: ctx.from?.id }, 'Admin bot error')
  })
