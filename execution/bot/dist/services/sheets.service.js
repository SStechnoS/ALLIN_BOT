...
const SHEETS_ID = config_1.config.GOOGLE_SHEETS_ID;
...
class SheetsService {
    sheets = googleapis_1.google.sheets({ version: 'v4', auth: getAuth() });
    rowToLead(row) {
        return {
            id: row[COLS.id] || '',
            ...
            source: row[COLS.source] || 'direct_bot',
    ...
    async findById(id) {
        ...
        await redis_1.redis.set(cacheKey, JSON.stringify(lead), 'EX', CACHE_TTL);
        return lead;
    }
    ...
    async updateField(leadId, field, value) {
        ...
        // Инвалидировать кэш
        await redis_1.redis.del(`lead:${leadId}`);
    }
