...
exports.aiGuard = {
    ...
    get INDIVIDUAL_RESPONSE() {
        return (`Индивидуальные занятия у нас тоже есть! 🎯\n\n` +
            ...
            `Для обсуждения деталей и записи напишите нашему менеджеру: ${config_1.config.MANAGER_LINK}`);
    },
    ...
    get RATE_LIMIT_RESPONSE() {
        return `Вы задали много вопросов подряд! Попробуйте через час 😊\nИли напишите менеджеру: ${config_1.config.MANAGER_LINK}`;
    },
    preFilter(text) {
        const lower = text.toLowerCase();
        if (INJECT_PATTERNS.some(kw => lower.includes(kw)))
            return 'inject';
        if (INDIVIDUAL_KEYWORDS.some(kw => lower.includes(kw)))
            return 'individual';
    ...
    isAITrigger(text, sceneId) {
        const strictScenes = ['phone', 'email', 'name'];
        if (strictScenes.includes(sceneId))
            return false;
        const patterns = [
