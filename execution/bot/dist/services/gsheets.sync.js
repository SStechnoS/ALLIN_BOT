...
exports.gsheetsSyncService = void 0;
/**
...
class GSheetsSyncService {
    ...
    getApi() {
        ...
        if (!config_1.config.GOOGLE_SERVICE_ACCOUNT_JSON || !config_1.config.GOOGLE_SHEETS_ID)
            return null;
        try {
            const creds = JSON.parse(config_1.config.GOOGLE_SERVICE_ACCOUNT_JSON);
    ...
    async findRowById(sheets, leadId) {
        const res = await sheets.spreadsheets.values.get({
            spreadsheetId: config_1.config.GOOGLE_SHEETS_ID,
    ...
    async syncLead(lead) {
        ...
        try {
            ...
            logger_1.logger.warn({ err, leadId: lead.id }, 'GSheets sync failed (non-fatal)');
        }
    }
}
exports.gsheetsSyncService = new GSheetsSyncService();
//# sourceMappingURL=gsheets.sync.js.map