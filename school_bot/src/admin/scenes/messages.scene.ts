...
import { getBotMessage, setBotMessage, type BotMessageKey } from '../../services/bot-messages.service';
...
export const adminMessagesScene = new Scenes.BaseScene<AdminBotContext>(SCENE_ADMIN_MESSAGES);
...
adminMessagesScene.action('msg_edit_welcome', async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    {
      parse_mode: 'HTML',
      ...Markup.inlineKeyboard([[Markup.button.callback('↩️ Отмена', 'msg_cancel_edit')]]),
    },
  );
});
...
adminMessagesScene.on('message', async (ctx) => {
  ...
  if (state.editing === 'welcome_video_note_id') {
    ...
    setBotMessage('welcome_video_note_id', ctx.message.video_note.file_id);
    ctx.scene.state = {};
